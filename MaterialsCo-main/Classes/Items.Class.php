...
class Items extends Dbh {
    ...
    protected function deleteReservationDB($ID) {
        ...
        // Get all active organizations (for browsing)
    ...
    protected function getMyOrganizations($userID) {
        $query = "
            SELECT o.*, u.NAME AS CREATOR_NAME
            FROM organizations o
            JOIN user u ON o.USER_ID = u.USER_ID
    ...
    protected function insertOrganization($userID, $name, $address, $type) {
        $query = "INSERT INTO organizations (USER_ID, NAME, ADDRESS, TYPE) VALUES (?, ?, ?, ?)";
        $stmt = $this->connection()->prepare($query);
        if (!$stmt->execute([$userID, $name, $address, $type])) { return false; }
        return true;
    }

    // Soft delete an organization (only owner can delete)
    protected function deleteOrganizationDB($organizationID, $userID) {
        $query = "UPDATE organizations SET IS_ACTIVE = 0 WHERE ORGANIZATION_ID = ? AND USER_ID = ?";
        $stmt = $this->connection()->prepare($query);
        if (!$stmt->execute([$organizationID, $userID])) { return false; }
