...
class ItemsCntrl extends Items {
    ...
    // Add Organization
    public function addOrganization($userID, $name, $address, $type) {
        if (empty($userID) || empty($name) || empty($address) || empty($type)) {
            return false;
        }
        return $this->insertOrganization($userID, $name, $address, $type);
    }

    // Delete Organization
    public function deleteOrganization($organizationID, $userID) {
        if (empty($organizationID) || empty($userID)) {
            return false;
        }
        return $this->deleteOrganizationDB($organizationID, $userID);
    }
}
